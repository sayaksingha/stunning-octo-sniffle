...
/*
typedef struct {
   UINT32  errorCode;   // Private: (Family<<8)|Specifier, National: integer code
   bool    isPrivate;   // true = Private (Tag 0xD3), false = National (Tag 0xD2)
} TCAPErrorCode;
*/
...
typedef struct {
  ...
  // UINT8  globalValue[TCAP_OPERATION_MAX_OID];
   ...
   // Ab Comment : Production fix for Aculab lazy-serialization trap.
   // acu_tcap_msg_add_comp_invoke() stores the op_code pointer INTERNALLY
   // in the acu_tcap_msg_t struct without copying the bytes. The actual BER
   // encoding only happens when acu_tcap_msg_send() is called from the
   // CALLER's context. Any stack-local UINT8[2] array passed as op_code
   // will be a dangling pointer by that point, corrupting the wire bytes.
   // wireOpCode[] is stored here — inside TCAPOperation which lives in
   // AnsiTcapMsg on the caller's stack — guaranteeing it is valid for the
   // entire Add -> Send sequence.
   // wireOpCode[0] = Family (upper byte),  wireOpCode[1] = Specifier (lower byte)
   // Valid only when isPrivate == true. Zero-initialised otherwise.
   UINT8    wireOpCode[2];
} TCAPOperation;
