...
struct chan {
	...
	struct sm_bfile_replay_parms rp;
	char *file;
	tSMModuleId module;
	MVIP xts;
};
 ...
 if (err) {
	...
	if (!fn_sm_switch_channel_output(ap.channel, cp)) {
		...
		if (lockadj(cp)) err = 1;
			...
			err = sm_replay_wav_bf_start(cp->file, &cp->rp);
			if (err) {
				fprintf(stderr, "%d.%d: sm_replay_wav_bf_start returned %d\n", cp->xts.stream, cp->xts.timeslot, err);
			}
		...
		if (!err) {
			struct timeb start;
			ftime(&start);
			err = sm_replay_bfile_progress(&cp->rp);
			if (err == ERR_SM_PENDING) {
				err = sm_replay_bfile_complete(&cp->rp);
			}
			cp->status = STS_STOPPING;
			if (err) {
				fprintf(stderr, "%d.%d: sm_bfile_replay_complete returned %d\n", cp->xts.stream, cp->xts.timeslot, err);
			} else {
				struct timeb now;
				double dtim;
				ftime(&now);
			...
			e2 = sm_replay_wav_bf_close(&cp->rp);
			if (!err) err = e2;
		}
	}
	...
	case 'v':
 	...
 	case 'q':
		...
		if (chan.status == STS_RUNNING) {
			sm_replay_bfile_stop(&chan.rp, 0);
...
int main(int argc, char **argv)
{
 ...
 iErr = playwav(arg.volume, arg.speed, arg.agc, arg.timeslot, mod,
	*argv);
 if (arg.serialnumber) {
	modclose(mod);
	cardclose(card);
