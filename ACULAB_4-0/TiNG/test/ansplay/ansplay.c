/* ansplay.c - test/demo of answering incoming calls and playing a message */
...
static int switch_chan(tSMChannelId chan, struct chan *cp)
{
 ...
 if (err) {
	fprintf(stderr, "%s: sm_switch_channel_output returned %d\n",
		cp->dbgname, err);
	return 1;
 }
 ...
 op.ost = call_port_2_stream(cp->port);
 if (op.ost < 0) {
 ...
 op.ist = cp->xts.stream;
 op.its = cp->xts.timeslot;
...
static int get_replay_event(tSMEventId *evp, tSMChannelId chan, struct chan *cp)
{
...
static int start_replay(tSMChannelId chan, struct chan *cp)
{
 ...
 if (err) {
	fprintf(stderr, "%s: sm_replay_start returned %d\n", cp->dbgname, err);
	return 1;
 }
 return 0;
}
...
static int do_replay(REPLAY *play, tSMEventId ev, struct chan *cp, struct state_xparms *cxp)
{
 ...
 for (;;) {
	 ...
	 * waiting on the event as it'll be set continuously until the
	 * channel has been filled with data. Version 1, however, will not
	 * set the event until the channel consumes data, so we must
	 * check first and fill it.
 	 */
	int err = checkplay(play);
	if (err) return 1;
	if (play->pstate == PLAY_EOF) {
		if (!cp->once) {
			off_t pos;
			err = bfseek(play->bf, &pos, 0, 0);
			if (err) return 1;
			play->pstate = PLAY_RUN;
		} else {
			SM_TS_DATA_PARMS ds;
			memset(&ds, 0, sizeof(ds));
			ds.channel = play->chan;
			ds.length = 0;
			err = sm_put_last_replay_data(&ds);
			if (err) {
				fprintf(stderr, "%s: sm_put_last_replay_data returned %d\n", cp->dbgname, err);
				return 1;
			}
		}
	}
	if (play->pstate == PLAY_DONE) {
		printf("done\n");
		return 0;
	}
	if (running) {
		struct timeb now;
		unsigned nbytes = play->bytes - prevbytes;
		double dtim;
		ftime(&now);
		...
		printf("\r%c %d  %d +/- %d bps  ",
			"|/-\\"[twid++ & 3], play->bytes,
			(int) ((8 * play->bytes + 4 * nbytes) / dtim),
			(int) (4 * nbytes / dtim));
		prevbytes = play->bytes;
		fflush(stdout);
		cxp->timeout = 0;
		cxp->handle = cp->handle;
		err = call_event(cxp);
		if (err) {
			fprintf(stderr, "%s: call_event returned %s\n", cp->dbgname, error_2_string(err));
			return 1;
		}
		if (cxp->handle){
			if (cxp->state != EV_CALL_CONNECTED) {
				SM_REPLAY_ABORT_PARMS rp;
				memset(&rp, 0, sizeof(rp));
				rp.channel = play->chan;
				err = sm_replay_abort(&rp);
				if (err) {
					fprintf(stderr, "%s: sm_replay_abort returned %d\n", cp->dbgname, err);
					return 1;
				}	
			}
			printf("abandoning\n");
		...
		printf("\r%c %d  ", ".*"[twid++ & 1], play->bytes);
		fflush(stdout);
	}
	err = smd_ev_wait(ev);
	if (err) {
...
static int playmsg(struct chan *cp, struct state_xparms *cxp)
{
 ...
 play.dbgname = cp->dbgname;
 // play cp->file to timeslot xp.stream:xp.ts
 err = bfile(&play.bf);
 if (err) {
	fprintf(stderr, "%s: bfile() failed: %d: %s\n",
		play.dbgname, err, strerror(err));
	return 1;
 }
 err = bfopen(play.bf, cp->file, "rb");
 if (err) {
	fprintf(stderr, "%s: bfopen(%s) failed: %d: %s\n",
		play.dbgname, cp->file, err, strerror(err));
	bfile_dtor(play.bf);
	return 1;
 }
 err = alloc_chan(&play.chan, cp);
 if (!err) {
 	err = switch_chan(play.chan, cp);
 	if (!err) {
		tSMEventId ev;
		err = get_replay_event(&ev, play.chan, cp);
		if (!err) {
			err = start_replay(play.chan, cp);
			if (!err) err = do_replay(&play, ev, cp, cxp);
			err |= free_ev(ev, cp);
		}
		err |= unswitch_chan(cp);
 	}
	err |= unalloc_chan(play.chan, cp);
 }
 err |= bfile_dtor(play.bf);
 return err;
}
...
recheck:
		...
		case EV_CALL_CONNECTED:
			...
			if (playmsg(cp, &stx)) return (void *) 1;
			if (stx.state != EV_CALL_CONNECTED) {
				goto recheck; // call state change stopped replay
			} else {
...
static int ansplay(ACU_CARD_ID card, unsigned numports, CARDLIST *cardsp, int once, int volume, int speed, unsigned agc, MVIP ts, enum kSMDataFormat format, unsigned rate, unsigned per_mod, char *file)
{
	ACU_PORT_ID *ports = 0;
	unsigned port;
	unsigned u;
 ...
 unsigned curmodpos = 0;		// index into mods[]
 unsigned thismod = 0;
 ...
 for (;;) {
	 ...
	 INIT_ACU_CL_STRUCT(&xp)
			...
			err=call_disconnect(&xp);
			if (err) {
				fprintf(stderr, "%s: call_disconnect returned %s\n", chans->dbgname, error_2_string(err));
				return 1;
			}
...
// open call
 ...
 e = ansplay(ocparms.card_id, cdiparms.ports, &cards, arg.once, arg.volume, arg.speed, arg.agc, arg.timeslot, format, rate, arg.per_mod, *argv);
// close things
 {
	// call
