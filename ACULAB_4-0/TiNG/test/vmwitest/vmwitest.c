/* vmwitest.c - test ADSI functions */
...
static int play_vmwi(tSMChannelId chan, int onhook, int onmsg, int smdf)
{
 SM_PLAY_VMWI_FSK_PARMS pvfp;
 int e;
 pvfp.channel = chan;
 pvfp.do_on_hook = onhook;
 pvfp.is_indicator_on_message = onmsg;
 pvfp.is_sdmf_message = smdf;
 e = sm_play_vmwi_fsk(&pvfp);
 if (e) {
 	fprintf(stderr, "sm_play_vmwi_fsk() returned %d\n", e);
	return 1;
 }
 return 0;
}
...
static int vmwitest(tSMModuleId module, MVIP ts, int sastype, int onhook, int smdf, int onmsg)
{
 ...
 if (play_vmwi(chan, onhook, onmsg, smdf)) return 1;
 for (;;) {
...
int main(int argc, char **argv)
{
 ...
 e = vmwitest(mod, arg.timeslot, arg.sastype, arg.onhook, arg.sdmf, arg.onmsg);
 if (arg.serialnumber) {
	modclose(mod);
	cardclose(card);
 }
