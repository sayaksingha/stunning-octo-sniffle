...
static void CreateDenseGrid(int r, int numtaps, int numband, REMEZ_BAND band[],
                     int *gridsize,
 ...
 */
   ...
   if ((symmetry == NEGATIVE) && (delf > band[0].lowf))
      band[0].lowf = delf;
...
static void Search(int r, int Ext[],
 ...
 *********************/
static void FreqSample(int N, double A[], float h[], int symm)
{
   int n, k;
   double x, val, M;
 ...
 */
   ...
   for (i=0; i<numband; i++)
   {
      gridsize += (int)(2*r*GRIDDENSITY*(band[i].highf - band[i].lowf) + .5);
   }
   if (symmetry == NEGATIVE)
   {
      gridsize--;
 ...
 */
   CreateDenseGrid(r, numtaps, numband, band,
