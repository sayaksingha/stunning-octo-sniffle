...
void MD5Update64(MD5_CTX *context, const unsigned char *input)
{
 context->count += 512;
 ...
 */
void MD5Final(unsigned char digest[16], MD5_CTX *context, const unsigned char *rem, unsigned len)
{
  unsigned char residue[64];
  uint64_t bits;
  while (len >= 64) {
	MD5Update64(context, rem);
	rem += 64;
	len -= 64;
  }
  memset(residue+len, 0, 64 - len);
  residue[len] = 0x80;
  if (len) {
	memcpy(residue, rem, len);
	if (len >= 56) {
		MD5Transform(context->state, residue);
		memset(residue, 0, len+1);
	}
  }
  bits = context->count + (len << 3);
	  /* Save number of bits */
  residue[56] = bits & 0xff;	bits >>= 8;
  residue[57] = bits & 0xff;	bits >>= 8;
  residue[58] = bits & 0xff;	bits >>= 8;
  residue[59] = bits & 0xff;	bits >>= 8;
  residue[60] = bits & 0xff;	bits >>= 8;
  residue[61] = bits & 0xff;	bits >>= 8;
  residue[62] = bits & 0xff;	bits >>= 8;
  residue[63] = bits & 0xff;
  MD5Transform(context->state, residue);
