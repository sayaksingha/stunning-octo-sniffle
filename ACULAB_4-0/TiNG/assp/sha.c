 ...
 * void SHATransform(uint_fast32_t digest[SHA_HASH_BUFFER_SIZE + SHA_HASH_EXTRA_SIZE],
 * 		uint_fast32_t const data[16])
 * 
 * The function hashes the input data to produce a digest in the first
 * SHA_HASH_BUFFER_SIZE words of the digest[] array, and uses SHA_HASH_EXTRA_SIZE
...
void SHATransform(uint_fast32_t digest[85], uint_fast32_t const data[16])
{
    uint_fast32_t A, B, C, D, E;     /* Local vars */
    uint_fast32_t TEMP;
    int	i;
#define W (digest + SHA_HASH_BUFFER_SIZE)	/* Expanded data array */
     ...
     */
    memcpy(W, data, 16*sizeof(uint_fast32_t));
    for (i = 0; i < 64; i++) {
